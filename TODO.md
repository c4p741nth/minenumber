...
- **B7 — มือการ์ดล้นจอแนวนอน**
  `Hand` เป็น `fixed inset-x-0` + flex แถวเดียว ไม่มี wrap/scroll
  พอมือถึง ~20 ใบ (default = ถือไม่จำกัด) การ์ดล้นออกนอกจอทั้งซ้ายและขวา กดใบท้าย ๆ ไม่ได้
  **แก้:** ย่อขนาดการ์ดอัตโนมัติตามจำนวนใบ (64 → 48px ที่ >12 ใบ → 36px ที่ >20 ใบ
  พร้อมย่อ font/emoji ด้วย `compact`) แล้วห่อด้วย `w-full overflow-x-auto` + แถวใน `w-max`
  → เกินจอเมื่อไหร่ก็ scroll แนวนอนได้ แต่ยังเป็นแถวเดียวเสมอ (ไม่ wrap ขึ้นไปบังกระดาน)
  — `src/components/cards/Hand.tsx`

- **B8 — กระดาน 200 ช่องสูงมาก ต้องเลื่อนจอเยอะ**
  ...
  **แก้:** เพิ่ม `cellSizeFor(count)` ย่อขนาดช่องขั้นบันได (≤60 ช่อง = 56px, 61–120 = 48px,
  >120 = 40px) → ที่ 200 ช่องได้คอลัมน์ต่อแถวมากขึ้น และให้ Board scroll ในกรอบตัวเอง
  (`max-h-[calc(100vh-14rem)] overflow-y-auto`) แทนการดันหน้าให้ยาว — มือการ์ดจึงไม่บังกระดาน
  ขนาดช่องคุมด้วย `style={{ minHeight: size }}` + font ย่อตาม (`small`)
  — `src/components/board/Board.tsx` + เทส `Board.test.ts` (4 ข้อ)
...
- `bun test` ผ่าน 125/125, `tsc --noEmit` ผ่าน, `bun run build` ผ่าน, console ไม่มี error
